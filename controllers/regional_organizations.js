...
const getOrganizations = async (req, res, next) => {
  try {
    ...
    const collaborator = await models.Collaborator.query()
      .withGraphFetched("organization")
      .where("account_id", account_id)
      .first();

    if (!collaborator && scope !== "ADMIN")
      return res.status(400).json("Invalid collaborator").send();
    ...
    const organizations = await models.RegionalOrganization.query()
      ...
      .select([
        ...
        models.RegionalOrganization.relatedQuery("verification_logs")
          .count()
...
const inviteOrganization = async (req, res, next) => {
  try {
    ...
    // Create client
    const organization = await models.RegionalOrganization.query().insert({
      name,
      description,
    ...
    if (locations.length > 0) {
      await models.RegionalOrganizationLocation.query().insert(locations);
    }
    ...
    return res
      .status(201)
      .json("Organization successfully created and invited. Waiting for signup")
      .send();
  } catch (e) {
    console.log(e);
    return res.status(500).json(JSON.stringify(e)).send();
...
const changePrimaryLocation = async (req, res, next) => {
  try {
    ...
    if (!collaborator && scope !== "ADMIN")
      return res.status(400).json("Invalid Role").send();
    if (
      collaborator &&
      collaborator.organization &&
      collaborator.organization.id !== Number(regional_organization_id)
    )
      return res.status(400).json("Invalid Organization").send();
...
const inviteCollaborator = async (req, res, next) => {
  try {
    ...
    if (!email || !role_id || !regional_organization_id)
    ...
    if (invitation)
      return res
        .status(400)
        .json("A pending invitation already exists with this email")
        .send();
    ...
    const organization = await models.RegionalOrganization.query()
      ...
      .modifyGraph("collaborator_invitations", (builder) => {
        builder.where(
          "collaborator_invitations.expiration_date",
          ">",
          new Date()
        );
      })
    ...
    if (!organization)
      return res.status(400).json("Invalid regional_organization_id").send();

    // Spaces left condition
    const invites_left = organization.collaborator_invitations.filter(
      (invite) => invite.status === "PENDING"
    );
    const space_occupied =
      organization.collaborators.length + invites_left.length;
    ...
    const role = await models.Role.query().where("id", role_id).first();
    ...
    // Send invite email
    await organizationInviteEmail(email, new_token, role, {
      name: display_name,
      custom_message,
      host,
    });
...
const revokeCollaboratorInvite = async (req, res, next) => {
  try {
    ...
    if (!collaborator && scope !== "ADMIN")
      return res.status(400).json("Invalid collaborator id").send();

    const collaborator_invitation = await models.CollaboratorInvitation.query().findById(
      Number(collaborator_invitation_id)
    );

    if (!collaborator_invitation)
      return res.status(400).json("Invalid invitation id").send();

    if (
      scope !== "ADMIN" &&
      collaborator &&
      collaborator_invitation.client_id !== collaborator.client_id
    )
      return res.status(400).json("You're not allowed to do this").send();

    await models.CollaboratorInvitation.query().deleteById(
...
const editSla = async (req, res, next) => {
  try {
    ...
    if (!regional_organization_id)
      return res.status(400).json("Invalid Organization");
...
const updatePrimaryLocation = async (req, res, next) => {
  try {
    ...
    const location_ids = organization.locations.map(
      (regional_location) => regional_location.location_id
    );

    if (location_ids.indexOf(location_id) < 0)
      return res.status(400).json("Invalid location").send();

    const collaborator = await models.Collaborator.query()
      .withGraphFetched("[account]")
      .findById(collaborator_id);
    if (!collaborator)
      return res.status(400).json("Invalid Collaborator").send();
    ...
    return res
      .status(200)
      .json("Succesfully updated collaborator primary location")
      .send();
  } catch (e) {
    console.log(e);
    return res.status(500).json(JSON.stringify(e)).send();
